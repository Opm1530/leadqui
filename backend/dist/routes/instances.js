...
const getEvolutionConfig = async (_userId) => {
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        const evoRes = await axios_1.default.post(`${baseUrl}/instance/create`, {
...
router.post("/:id/set-webhook", async (req, res) => {
    try {
        const instance = await prisma_1.default.instance.findUnique({ where: { id: String(req.params.id) } });
        if (!instance) {
        ...
        let lastErr = null;
        try {
            await axios_1.default.post(`${baseUrl}/webhook/set/${instance.evolution_instance_id}`, { webhook: { enabled: true, url, events, byEvents: false, base64: false } }, { headers: { apikey: apiKey, "Content-Type": "application/json" } });
            ok = true;
        }
        catch (e) {
            lastErr = e;
        ...
        if (!ok) {
            try {
                await axios_1.default.post(`${baseUrl}/webhook/set/${instance.evolution_instance_id}`, { url, webhook_by_events: false, events }, { headers: { apikey: apiKey, "Content-Type": "application/json" } });
                ok = true;
            }
            catch (e) {
                lastErr = e;
        ...
        if (!ok)
            throw lastErr || new Error("Falha ao configurar webhook");
...
router.get("/:id/qrcode", async (req, res) => {
    ...
    try {
        ...
        const { baseUrl, apiKey } = await getEvolutionConfig(req.user.id);
        const evoRes = await axios_1.default.get(`${baseUrl}/instance/connect/${instance.evolution_instance_id}`, { headers: { apikey: apiKey } });
...
router.get("/:id/status", async (req, res) => {
    ...
    try {
        ...
        const { baseUrl, apiKey } = await getEvolutionConfig(req.user.id);
        const evoRes = await axios_1.default.get(`${baseUrl}/instance/connectionState/${instance.evolution_instance_id}`, { headers: { apikey: apiKey } });
        ...
        await prisma_1.default.instance.update({ where: { id }, data: { status: newStatus } });
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        ...
        try {
            const { baseUrl, apiKey } = await getEvolutionConfig(req.user.id);
            await axios_1.default.delete(`${baseUrl}/instance/delete/${instance.evolution_instance_id}`, {
                headers: { apikey: apiKey },
            });
        }
        catch { /* ignore if Evolution fails */ }
        await prisma_1.default.instance.delete({ where: { id } });
