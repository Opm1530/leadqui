...
router.get("/dashboard", async (req, res) => {
    try {
        ...
        const paidThisMonth = invoices
            .filter((i) => i.status === "PAGO" && i.paid_date >= startOfMonth && i.paid_date <= endOfMonth)
        ...
        const mrrTotal = contracts.reduce((sum, c) => sum + Number(c.value), 0);
        res.json({
            mrr: mrrTotal,
            ...
            overdue_clients: invoices.filter((i) => i.status === "ATRASADO").length,
        });
    }
    catch (error) {
...
router.put("/invoices/:id", async (req, res) => {
    ...
    try {
        ...
        const updated = await prisma_1.default.invoice.update({
            ...
            data: {
                ...(status && { status }),
                ...(paid_date !== undefined && { paid_date: (0, dates_1.dayDate)(paid_date) }),
                ...(description !== undefined && { description }),
                ...(amount !== undefined && { amount: parseFloat(amount) }),
                ...(due_date !== undefined && { due_date: (0, dates_1.dayDate)(due_date) }),
            },
            include: { client: { select: { name: true } } },
        });
        ...
        if (status === "ATRASADO") {
            ...
            const hasOtherOverdue = await prisma_1.default.invoice.findFirst({
    ...
    try {
        ...
        const expense = await prisma_1.default.expense.update({
            ...
            data: {
                ...
                ...(amount !== undefined && { amount: parseFloat(amount) }),
                ...(category && { category }),
                ...(date && { date: (0, dates_1.dayDate)(date) }),
            },
        });
        res.json({ expense });
    }
...
router.get("/report", async (req, res) => {
    try {
        ...
        for (let m = 0; m < 12; m++) {
            ...
            const revenue = invoicesPaid.reduce((sum, i) => sum + i.amount, 0);
            const expenses = expensesMonth.reduce((sum, e) => sum + e.amount, 0);
            months.push({
                month: m + 1,
                revenue,
                expenses,
...
router.get("/fixed-expenses", async (req, res) => {
    try {
        const items = await prisma_1.default.fixedExpense.findMany({
            orderBy: { due_day: "asc" },
        });
...
router.post("/fixed-expenses", async (req, res) => {
    ...
    try {
        const item = await prisma_1.default.fixedExpense.create({
            data: {
                user_id: req.user.id,
                description,
                amount: parseFloat(amount),
...
router.put("/fixed-expenses/:id", async (req, res) => {
    ...
    try {
        const existing = await prisma_1.default.fixedExpense.findFirst({ where: { id } });
        if (!existing) {
            res.status(404).json({ error: "Não encontrado" });
            return;
        }
        const item = await prisma_1.default.fixedExpense.update({
            where: { id },
            data: {
                ...(description && { description }),
                ...(amount !== undefined && { amount: parseFloat(amount) }),
...
router.delete("/fixed-expenses/:id", async (req, res) => {
    ...
    try {
        ...
        await prisma_1.default.fixedExpense.delete({ where: { id } });
        res.json({ success: true });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.post("/inter/credentials", async (req, res) => {
    ...
    try {
        const creds = await prisma_1.default.interCredentials.upsert({
            update: {
                client_id,
                client_secret,
...
router.get("/inter/credentials", async (req, res) => {
    try {
        const creds = await prisma_1.default.interCredentials.findUnique({
...
router.post("/inter/sync", async (req, res) => {
    try {
        const creds = await prisma_1.default.interCredentials.findUnique({});
        if (!creds) {
            res.status(400).json({ error: "Credenciais Inter não configuradas" });
            return;
        }
        ...
        const tokenRes = await axios_1.default.post("https://cdpj.partners.bancointer.com.br/oauth/v2/token", new URLSearchParams({
            client_id: creds.client_id,
        ...
        const extratoRes = await axios_1.default.get("https://cdpj.partners.bancointer.com.br/banking/v2/extrato", {
        ...
        for (const tx of transactions) {
            ...
            await prisma_1.default.interTransaction.create({
                data: {
        ...
        await prisma_1.default.interCredentials.update({
...
router.get("/inter/transactions", async (req, res) => {
    const { status } = req.query;
    try {
        const creds = await prisma_1.default.interCredentials.findUnique({ where: { user_id: req.user.id } });
        if (!creds) {
            res.json({ transactions: [] });
            return;
        }
        const where = { credentials_id: creds.id };
        if (status)
            where.status = status;
        const transactions = await prisma_1.default.interTransaction.findMany({
            where,
            orderBy: { date: "desc" },
        });
...
router.patch("/inter/transactions/:id/link", async (req, res) => {
    const { id } = req.params;
    const { status, linked_invoice_id, linked_expense_id } = req.body;
    try {
        const tx = await prisma_1.default.interTransaction.update({
            where: { id },
            data: {
...
//# sourceMappingURL=cashqui.js.map