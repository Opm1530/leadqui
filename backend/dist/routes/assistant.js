...
function expandirRecorrencia(dataInicio, dataFim, regras) {
    const out = [];
...
async function execReadTool(name, args, userId) {
    ...
    if (name === "listar_calendario") {
        const where = { client_id: args.client_id };
...
router.post("/chat", async (req, res) => {
    ...
    if (!Array.isArray(messages)) {
        res.status(400).json({ error: "messages deve ser array" });
        return;
    }
    try {
        ...
        if (!apiKey) {
            res.status(400).json({ error: "Anthropic API Key não configurada nas Configurações." });
            return;
        }
...
async function buildProposal(fnName, args) {
    if (fnName === "criar_card") {
        const client = await prisma_1.default.client.findUnique({ where: { id: args.client_id }, select: { name: true } });
        const datas = Array.isArray(args.datas) ? args.datas : [args.datas].filter(Boolean);
        return {
    ...
    if (fnName === "preencher_card") {
        const post = await prisma_1.default.calendarPost.findUnique({ where: { id: args.post_id }, select: { scheduled_date: true } });
    ...
    if (fnName === "enviar_para_producao") {
        ...
        const trelloOn = await (0, trello_1.isTrelloConfigured)();
        return {
            id: Math.random().toString(36).slice(2),
    ...
    if (fnName === "limpar_calendario") {
        ...
        const count = await prisma_1.default.calendarPost.count({ where });
        return {
            id: Math.random().toString(36).slice(2),
    ...
    if (fnName === "criar_calendario_recorrente") {
        ...
        const resumo = (args.regras || [])
            .map((r) => `${r.dia_semana.toLowerCase()} → ${r.tipo}`)
            .join(", ");
...
router.post("/execute", async (req, res) => {
    ...
    try {
        ...
        if ((type === "criar_card" || type === "adicionar_conteudo" || type === "criar_calendario_recorrente")) {
            ...
            const cliente = await prisma_1.default.client.findUnique({ where: { id: payload.client_id }, select: { id: true } });
            if (!cliente) {
        ...
        if (type === "criar_card") {
            ...
            const created = await Promise.all(datas.map((d) => prisma_1.default.calendarPost.create({
                data: {
        ...
        if (type === "adicionar_conteudo") {
            const post = await prisma_1.default.calendarPost.create({
                data: {
                    ...
                    type: payload.type || "POST",
                    platform: payload.platform || "INSTAGRAM",
                    scheduled_date: (0, dates_1.dayDate)(payload.scheduled_date),
                    status: "PLANEJADO",
                },
        ...
        if (type === "enviar_para_producao") {
            const { post, trello, task } = await (0, production_1.sendPostToProduction)(payload.post_id);
            const extras = [
        ...
        if (type === "remover_card") {
            const post = await prisma_1.default.calendarPost.findUnique({ where: { id: payload.post_id }, select: { title: true } });
            await prisma_1.default.calendarPost.delete({ where: { id: payload.post_id } });
            res.json({ success: true, message: `Card "${post?.title || payload.post_id}" removido do calendário.` });
            return;
        }
        if (type === "limpar_calendario") {
            const where = { client_id: payload.client_id };
            ...
            const result = await prisma_1.default.calendarPost.deleteMany({ where });
        ...
        if (type === "criar_calendario_recorrente") {
            ...
            if (!itens.length) {
                res.json({ success: true, message: "Nenhuma data correspondente às regras no período." });
                return;
            }
            const result = await prisma_1.default.calendarPost.createMany({
                data: itens.map(it => ({
                    client_id: payload.client_id,
                    title: null,
                    type: it.tipo,
