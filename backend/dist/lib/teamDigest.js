...
exports.sendTeamDigest = sendTeamDigest;
exports.sendTeamDigestTest = sendTeamDigestTest;
exports.startTeamDigest = startTeamDigest;
...
const SAUDACAO = { 6: "☀️ Bom dia, equipe!", 12: "🌤️ Boa tarde, equipe!", 18: "🌙 Fim de dia, equipe!" };
const LINHA = "━━━━━━━━━━━━━━";
...
async function postToGroup(text) {
    ...
    await axios_1.default.post(`${s.evolution_api_url.replace(/\/$/, "")}/message/sendText/${s.notification_instance}`, { number: s.notification_group_id, text }, { headers: { apikey: s.evolution_api_key || "", "Content-Type": "application/json" }, timeout: 8000 });
}
// Monta a mensagem do boletim. Retorna null se não há nada para enviar.
async function buildDigest(hour) {
    ...
    if (!reminders.length && !tasks.length)
        return null;
...
async function sendTeamDigest(hour) {
    try {
...
//# sourceMappingURL=teamDigest.js.map