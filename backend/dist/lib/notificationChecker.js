...
exports.NOTIF_TYPES = void 0;
exports.checkNotifications = checkNotifications;
exports.startNotificationChecker = startNotificationChecker;
...
async function createNotif(userId, type, title, message, link, referenceId) {
    ...
    if (existing)
        return null; // já foi criada hoje
...
async function sendWhatsApp(message) {
    try {
        ...
        if (!settings?.evolution_api_url || !settings?.notification_phone || !settings?.notification_instance)
            return;
        await axios_1.default.post(`${settings.evolution_api_url}/message/sendText/${settings.notification_instance}`, { number: settings.notification_phone, text: message }, {
...
async function checkNotifications() {
    try {
        ...
        const notifyAll = async (type, title, message, link, refId) => {
            let isNew = false;
        ...
        for (const inv of overdueInvoices) {
            ...
            const isNew = await notifyAll(exports.NOTIF_TYPES.FATURA_ATRASADA, `Fatura atrasada — ${clientName}`, `R$ ${valor} com ${daysLate} dia${daysLate > 1 ? "s" : ""} de atraso`, "/cashqui/invoices", inv.id);
            if (isNew)
                await sendWhatsApp(`🔴 *Fatura Atrasada* — ${clientName}\nR$ ${valor} · ${daysLate} dia${daysLate > 1 ? "s" : ""} de atraso`);
        }
        // ── 2. Tarefas vencidas ───────────────────────────────────────────
        ...
        for (const task of overdueTasks) {
            ...
            await notifyAll(exports.NOTIF_TYPES.TAREFA_VENCIDA, `Tarefa vencida — ${task.title}`, `${task.client?.name || ""} · venceu há ${daysLate} dia${daysLate > 1 ? "s" : ""}`, "/tasqui", task.id);
        }
        // ── 3. Despesas fixas vencendo nos próximos 3 dias ────────────────
        const currentDay = today.getDate();
        const fixedExpenses = await prisma_1.default.fixedExpense.findMany({
        ...
        for (const exp of fixedExpenses) {
            ...
            const isNew = await notifyAll(exports.NOTIF_TYPES.DESPESA_FIXA_VENCENDO, `Despesa fixa vencendo${daysUntil === 0 ? " hoje" : ` em ${daysUntil} dia${daysUntil > 1 ? "s" : ""}`}`, `${exp.description} · R$ ${valor}`, "/cashqui/fixed-expenses", refId);
            if (isNew && daysUntil === 0)
                await sendWhatsApp(`🟡 *Despesa Fixa Hoje* — ${exp.description}\nR$ ${valor}`);
        }
        // ── 4. Posts aguardando aprovação ─────────────────────────────────
        const tomorrow = new Date(today);
        tomorrow.setDate(tomorrow.getDate() + 1);
        tomorrow.setHours(23, 59, 59, 999);
        const pendingPosts = await prisma_1.default.calendarPost.findMany({
            where: { status: { in: ["PLANEJADO", "PRODUZINDO"] }, scheduled_date: { lte: tomorrow } },
            include: { client: { select: { name: true } } },
        });
        for (const post of pendingPosts) {
            const clientName = post.client?.name || "Cliente";
            const isNew = await notifyAll(exports.NOTIF_TYPES.POST_APROVACAO, `Post aguardando aprovação — ${clientName}`, `"${post.title}" · ${post.type} · ${post.platform}`, "/tasqui/calendar", post.id);
            if (isNew)
                await sendWhatsApp(`📅 *Post Aguardando Aprovação* — ${clientName}\n"${post.title}" · ${post.type} · ${post.platform}`);
        }
    }
    catch (error) {
...
//# sourceMappingURL=notificationChecker.js.map